...
class ServidorHttp{
    ...
    private void ProcessarRequest(Socket conexao, int numeroRequest)
    {
        ...
        if(conexao.Connected)
        {
            ...
            if(textoRequisicao.Length > 0)
            {
                    ...
                    //verifica se contem duas quebras de linhas e se tiver logo apos vai ter os dados postados do 
                    //formulario que sera capiturados na string a baixo
                    string dadosPoost = textoRequisicao.Contains("\r\n\r\n") ? textoRequisicao.Split("\r\n\r\n")[1] : "";
                    recursoBuscado = recursoBuscado.Split("?")[0];
                    //verifica se ha dados e os decodifcam
                    if(!string.IsNullOrEmpty(dadosPoost))
                    {
                        dadosPoost = HttpUtility.UrlDecode(dadosPoost, Encoding.UTF8);
                        var parametrosPost = ProcessarParametros(dadosPoost);
                        //abaixo seram adicionados os parametros do post a lista
    ...
    //metedo inicial usado de forma a exemplificar a criação da estrutura de um arquivo
    //html atraves do elemento StringBuilder
...
//metodo que era responsavel para ler o arquivo html e retornar o conteudo em bytes
...
//  }
    ...
    {
        //pegar a informações de caminhoArquivo armazenando no objeto fiArquivo
        //é porssivel extrair desse obejto fiArquivo o nome da pagina solicitada
        FileInfo fiArquivo = new FileInfo(caminhoArquivo);
        //pega o nome do arquivo sem extensao e adiciona a string "Pagina"
        string nomeClassePagina = "Pagina" + fiArquivo.Name.Replace(fiArquivo.Extension, "");
        //pega o tipo da pagina a partir do nome do tipo da pagina
        Type tipoPaginaDinamica = Type.GetType(nomeClassePagina, true, true);
        //aqui é criada a instancia da classe relacionada a classe PaginaDinamica
        PaginaDinamica pd = Activator.CreateInstance(tipoPaginaDinamica) as PaginaDinamica;
        //aqui o objto sera alimentado com o conteudo de htmlModelo
